# Azure Service Bus and Function App Creation Script
# This script extends the existing infrastructure with Service Bus and Azure Functions
# Uses basic/free tiers suitable for Visual Studio subscriptions
...
param(
    ...
    [string]$TopicName = "weather-updates",
    [string]$SubscriptionName = "processing-subscription",
...
try {
    ...
    # Step 1: Create Storage Account for Function App (required)
    Write-Host "`nCreating Storage Account for Function App..." -ForegroundColor Yellow
    ...
    # Step 2: Create Service Bus Namespace (Basic tier - lowest cost)
    ...
    # Step 3: Create Service Bus Topic
    ...
    # Step 4: Create Service Bus Subscription
    ...
    # Step 6: Get Application Insights Connection String (if exists)
    ...
    # Step 7: Create Function App (Consumption plan - pay per execution)
    ...
    & $azPath functionapp config appsettings set `
        --name $FunctionAppName `
        --resource-group $ResourceGroupName `
        --settings "ServiceBusConnectionString=$serviceBusConnectionString" `
    ...
    Write-Host "Resource Group: $ResourceGroupName"
    Write-Host "Service Bus Namespace: $ServiceBusNamespace"
    Write-Host "Service Bus Topic: $TopicName"
    Write-Host "Service Bus Subscription: $SubscriptionName"
    Write-Host "Function App: $FunctionAppName"
...
Write-Host "💡 All resources use basic/free tiers suitable for Visual Studio subscriptions." -ForegroundColor Cyan
